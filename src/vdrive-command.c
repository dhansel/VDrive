...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "Command '%s' (%u) %d.", cmd.command==NULL ? "(NULL)" : (const char *) cmd.command, cmd.commandlength, status);
    log_debug(LOG_DEFAULT, "File    '%s' (%u).", cmd.file==NULL ? "(NULL)" : (const char *) cmd.file, cmd.filelength);
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "RENAME: dest name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "RENAME: src name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "RENAMEHEADER: dest name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "RENAMEPART: dest name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "RENAMEPART: src name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
#ifdef DEBUG_DRIVE
    log_debug(LOG_DEFAULT, "LOCK/UNLOCK: dest name='%s', len=%u", cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
/* backup current partition information */
    ...
    do {
        ...
        log_debug(LOG_DEFAULT, "remove name='%s', len=%u, type=%u.",
            cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength, cmd->filetype);
#endif
...
/* backup current partition information */
    ...
    log_debug(LOG_DEFAULT, "chpart name='%s', len=%u",
              cmd->file ? (const char *) cmd->file : "(NULL)", cmd->filelength);
#endif
...
int vdrive_command_format(struct vdrive_s *vdrive, const char *disk_name)
{
    ...
    if (vdrive->image->device == DISK_IMAGE_DEVICE_FS) {
      if( !archdep_fisopen(disk_image_fsimage_fd_get(vdrive->image)) ) {
            return CBMDOS_IPE_NOT_READY;
        }
    }
